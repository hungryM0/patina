...
该扩展包可以随 Patina Release 一起分发，但扩展内部 `manifest.json` 的 `version` 独立管理。只有扩展自身发生可发布变化时，才更新扩展版本号。
附件文件名中的 `vX.Y.Z` 必须来自扩展自己的 `manifest.json`，例如扩展版本为 `0.1.0` 时，附件名为 `patina-chromium-extension-v0.1.0.zip`。
zip 内部必须包含同名扩展目录，例如 `patina-chromium-extension-v0.1.0/`，并让 `README.en.md` 与 `README.zh-CN.md` 在 zip 根目录与该扩展目录并列，避免用户解压时把扩展运行文件散落到当前目录，同时方便先阅读使用说明。
用户安装时不能直接通过“加载未打包”选择 zip 文件；应先解压 zip，再在浏览器扩展页选择解压后包含 `manifest.json` 的目录。
...
- R2 不同步 `patina-chromium-extension-vX.Y.Z.zip`；浏览器扩展包只作为 GitHub Release 附件分发
...
`write-release-notes`、`npm run tauri build -- --bundles nsis`、`npm run extension:chromium:package` 与 `npm run release:prepare-assets`
...
8. 由工作流生成 release notes、构建安装包、打包 Chromium 扩展与 GitHub 版 `latest.json`。
9. 由工作流发布 GitHub Release，附件至少包含 Windows 安装包、`latest.json` 与 `patina-chromium-extension-vX.Y.Z.zip`。
