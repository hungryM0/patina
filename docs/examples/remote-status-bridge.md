...
远程推送把本机当前追踪状态发到用户自己的 `WebSocket` 服务，比如 Cloudflare Worker。

这条链路只发当前状态，不发历史记录，也不直接接入远端数据库。本项目只负责 `Windows -> Worker` 这一段。Worker 后面要怎么转发到网页或别的展示系统，由 Worker 项目自己处理。
