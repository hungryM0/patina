...
Time Tracker is a personal, local-first Windows desktop time tracker. The project
...
Time Tracker is intentionally focused on personal, local-first Windows desktop
...
```bash
git remote add upstream https://github.com/Ceceliaee/time-tracking.git
...
- Describe what is intentionally included.
- Describe important non-goals.
...
感谢你愿意为 Time Tracker 做出贡献。

Time Tracker 是一个面向个人使用、本地优先的 Windows 桌面时间追踪工具。
...
Time Tracker 有意聚焦个人、本地优先的 Windows 桌面使用场景。除非先明确调整项目方向，
